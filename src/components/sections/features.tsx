...
const features = [
  ...
  },
  {
    ...
    glowClass: 'hover:shadow-cyan-500/50',
    href: '/ar-simulation'
  },
  {
    icon: Brain,
...
export function Features() {
  return (
    // Fits inside wrapper without extra decorative background
    <section className="h-full flex flex-col justify-center">
      <div className="container mx-auto px-4">
        {/* Heading */}
        <div className="text-center mb-16">
          <h2 className="text-4xl lg:text-6xl font-bold mb-6 text-gradient-animated">
            Revolutionary Learning Features
          </h2>
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
            Cutting-edge technology meets educational excellence to create the most advanced learning platform ever built.
          </p>
        </div>
          ...
          {features.map((feature) => (
            <TiltCard key={feature.title} className="h-full">
              <motion.div
                ...
                className={cn(
                  "glass p-6 rounded-xl h-full flex flex-col justify-between shadow-lg transition-shadow duration-300",
                  feature.glowClass
                )}
                data-cursor-hover
              >
                ...
                {feature.href && (
                  <div className="mt-6">
                    <Link href={feature.href} passHref>
                      <Button className="w-full font-bold btn-primary-glow">
                        Try It Now
                      </Button>
                    </Link>
                  </div>
